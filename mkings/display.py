...
class Display:
                ...
                if not is_open:
                    if card.back == "WHITE":
                        white_back = self.get_image(
                            "white_back.png", self.CARD_SIZE, self.CARD_SIZE
                        )
                        self.WINDOW.blit(white_back, pos_on_screen)
                    elif card.back == "BLACK":
                        black_back = self.get_image(
                            "black_back.png", self.CARD_SIZE, self.CARD_SIZE
                        )
    ...
    def print_tokens(self):
        ...
        for player in Player.array:
            for token in player.token:
                pos_on_screen = (
                    int(
                        (self.CORNER[0] + self.CARD_SIZE * (1 + token.col))
                        - self.TOKEN_SIZE * 1.23
                    ),
                    int(
                        (self.CORNER[1] + self.CARD_SIZE * (token.row))
                        + self.TOKEN_SIZE * 0.32
                    ),
                )
                token_image = self.get_image(
                    token.image, self.TOKEN_SIZE, self.TOKEN_SIZE
                )
    ...
    def print_selected(self, current_player):
        ...
        if Pawn.selected:
            pos_on_screen = list(
                current_player._get_pawn_on_screen(self, Pawn.selected)
            )
    ...
    def print_eye(self, board, col, row):
        ...
        queen_advice = self.get_image(
            "queen_advice.png", self.CARD_SIZE, self.CARD_SIZE
        )
    ...
    def get_cursor_eye(self):
        ...
        return pygame.mouse.set_cursor(
            (32, 24), (15, 11), *pygame.cursors.compile(eye_cursor)
        )
