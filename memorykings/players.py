    ...
    def recruit(self, board, card_array):
        ...
        if self.pawn[0].position != self.pawn[1].position:
            if card_0.color == card_1.color and card_0.rank == card_1.rank:
                ...
                if card_0.get_token(Token.array) != None:
                    log.debug(f"recruit() - These cards were already" 
...
class CounterKing(Player):
    ...
    def recruit(self, board, card_array):
        ...
        for pawn in self.array[1].pawn:
            ...
            if (
                counter_card.get_token(Token.array) == None 
...
class Pawn:
    ...
    def get_screen_location(self, pawn_num, player_order):
        ...
        '''
        return (
        (CORNER[0]+CARD_SIZE*(self.col))+(PAWN_SIZE*pawn_num)+CARD_BORDER, 
        (CORNER[1]+CARD_SIZE*(self.row+1))-(PAWN_SIZE*(player_order))-CARD_BORDER
        )
